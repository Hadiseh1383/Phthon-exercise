...
def log_in(id,u_password):
    ...
    return menu
com= input().strip()
while com != 'edu exit edu':
    if re.fullmatch(r'edu sign up -.+ -i .+ -n .+ -p .+ edu',com) != None:
        a = re.fullmatch(r'edu sign up -(.+) -i (.+) -n (.+) -p (.+) edu',com)
        sign_up(a.group(1),a.group(2),a.group(3),a.group(4))
        com = input().strip()
    elif re.fullmatch(r'edu log in -i .+ -p .+ edu',com) != None:
        a = re.fullmatch(r'edu log in -i (.+) -p (.+) edu',com)
        menu = log_in(a.group(1),a.group(2))
        if menu != 'log in/sign up menu':
            com = input().strip()
            while com!= 'edu log out edu':
                if re.fullmatch(r'edu show course list edu',com) != None:
                    course_display()
                    com = input().strip()
                elif re.fullmatch(r'edu add course -c .+ -i .+ -n .+ edu',com) != None and menu == 'professor menu':
                    b = re.fullmatch(r'edu add course -c (.+) -i (.+) -n (.+) edu',com)
                    add_course(b.group(1),b.group(2),b.group(3))
                    com = input().strip()
                elif re.fullmatch(r'edu get course -i .+ edu',com) != None and menu == 'student menu':
                    b = re.fullmatch(r'edu get course -i (.+) edu',com)
                    student_dict[a.group(1)].get_course(b.group(1))
                    com = input().strip()                
                elif re.fullmatch(r'edu current menu edu',com) != None:                
                    print(menu)
                    com= input().strip()
                else:
